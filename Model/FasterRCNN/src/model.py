...
class MotionBlur(torch.nn.Module):
    ...
    def __init__(self, kernel_size, *args, **kwargs) -> None:
        ...
        self.logits = torch.nn.Parameter(torch.randn(1, 4))
        self.probs = F.gumbel_softmax(self.logits, tau=1, hard=False)

        # Initialize Final image
        self.final_image = torch.zeros(3, 512, 512)

    def forward(self, x):
        # Compute convolutions
        motion_east = F.conv2d(
            x, self.kernel_east, stride=(1), padding="same", groups=3
        )
        motion_west = F.conv2d(
            x, self.kernel_west, stride=(1), padding="same", groups=3
        )
        motion_south = F.conv2d(
            x, self.kernel_south, stride=(1), padding="same", groups=3
        )
        motion_north = F.conv2d(
            x, self.kernel_north, stride=(1), padding="same", groups=3
        )
        stack = torch.stack(
